...
export default function UpgradeMembership() {
  ...
  const handleCheckout = async () => {
    ...
    const res = await fetch("/api/membership/create-checkout-session", {//should be create-paystack-checkout-session.js
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        email: session.user.email,
    ...
    if (data.authorization_url) {
      ...
      setLoading(false);
      alert("Error creating Paystack checkout session.");
    }
  };
